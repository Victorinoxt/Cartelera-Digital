...
class EnvConfig {
  ...
  static String get localApiUrl =>
      dotenv.env['LOCAL_API_URL'] ?? 'http://localhost:3000/api';
  static String get externalApiUrl =>
      dotenv.env['EXTERNAL_API_URL'] ?? 'http://192.168.0.3:3003/api';
  static String get monitoringApiUrl =>
      dotenv.env['MONITORING_API_URL'] ?? 'http://192.168.0.4:3000/api';

  // External API Endpoints
  static String get planificadorEndpoint => '/planificador/graph_1';

  // WebSocket Configuration
  static bool get wsEnabled =>
      dotenv.env['WS_ENABLED']?.toLowerCase() == 'true';
  static String get wsPath => dotenv.env['WS_PATH'] ?? '/ws';
  static int get wsReconnectInterval =>
      int.parse(dotenv.env['WS_RECONNECT_INTERVAL'] ?? '5000');
  ...
  static int get cacheDurationMinutes =>
      int.parse(dotenv.env['CACHE_DURATION_MINUTES'] ?? '5');
  static bool get debugMode =>
      dotenv.env['DEBUG_MODE']?.toLowerCase() == 'true';
  static bool get enableImageCache =>
      dotenv.env['ENABLE_IMAGE_CACHE']?.toLowerCase() == 'true';
  static int get imageCacheSize =>
      int.parse(dotenv.env['IMAGE_CACHE_SIZE'] ?? '100');

  // Security Configuration
  static String get jwtSecret =>
      dotenv.env['JWT_SECRET'] ?? 'default-secret-key';
  ...
  static Future<void> init() async {
    try {
      ...
      try {
        ...
        print(
            'WARNING: No se pudo cargar el archivo .env desde los assets: $assetError');
